...
const exportScraper = async(results) => {
	...
	results.forEach(item => {
		worksheet.addRow({
			...
			oeBullPts: item.bulletPoint.oeBulletPoint,
			amzBullPts: item.bulletPoint.amazonBulletPoint,
...
const scraper = async () => {
    ...
    const prodDetails = [
		{ sku: 'FTM-Ariel', asin: 'B07NQQ573T'},
		{ sku: 'ECK-EC041269010', asin: 'B00M1QZBAK'},
		// { sku: 'BR5923-2XL', asin: 'B01N0EFKH1'},
		// { sku: 'OY-OO9188-03', asin: 'B00UT4E5EE'},
		// { sku: 'FTM-1120_699-Y3', asin: 'B0825BNJCL'} 
	]
	...
	for (let x in prodDetails) {
		...
		try {
			...
			const brandName = await page.evaluate(()=> {
				...
				amazonBrandName = document.querySelector('#attribute-group-brand > section > div > div > div.kat-row.kat-row-padding-top-bottom > section.kat-col-xs-7 > span')?document.querySelector('#attribute-group-brand > section > div > div > div.kat-row.kat-row-padding-top-bottom > section.kat-col-xs-7 > span').innerHTML:'n/a';
			...
			const bulletPoint = await page.evaluate(()=> {
				...
				if(document.querySelector('#attribute-group-bullet_point > section > div > div > div:nth-child(1) > div.kat-row.kat-row-padding-top-bottom > section.kat-col-xs-7 > span'.innerHTML)===null){
			...
			const genericKeywords = await page.evaluate(()=>{
				...
				if (document.querySelector('#attribute-group-generic_keyword > section > div > div > div:nth-child(1) > div.kat-row.kat-row-padding-top-bottom > section.kat-col-xs-7 > span')){
					amazonGenericKeywords= document.querySelector('#attribute-group-generic_keyword > section > div > div > div:nth-child(1) > div.kat-row.kat-row-padding-top-bottom > section.kat-col-xs-7 > span').innerHTML;
				}else if(document.querySelector('#attribute-group-generic_keyword > section > div > div > div:nth-child(1) > div.kat-row.kat-row-padding-top-bottom > section.kat-col-xs-7 > span').innerHTML === null){
					amazonGenericKeywords='n/a';
				}else{
					amazonGenericKeywords='n/a';
				}
